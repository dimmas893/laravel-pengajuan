...
class ApprovalController extends Controller
{
    ...
    public function approve_admin(Request $request)
    {
        ...
        $user = User::where('role', 'level_2')->get();
        // $pengajuan = Pengajuan::where('id', $request->id)->where('')->get();
        // $jsonNilai = array();
        // $nama = array();
        // foreach ($pengajuan_detail as $p) {
        ...
        $message = view('email_to_level_2', compact('pengajuan_detail', 'pengajuan', 'total'))->render();

        try {
            foreach ($user as $p) {
                $mail = new PHPMailer(true);
                $mail->isSMTP();
                $mail->Host = 'mail.cekud.my.id';
                $mail->SMTPAuth = true;
                $mail->Username = 'dimas@cekud.my.id';
                $mail->Password = 'SqT5~]Wt9#&S';
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;
